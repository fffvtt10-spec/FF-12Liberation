...
// Importamos a música global da LandingPage
...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Força o volume correto sempre que entrar nesta tela
    backgroundMusic.volume = 0.2;

    // Se por acaso a música não estiver tocando (ex: refresh), tenta tocar
    if (backgroundMusic.paused) {
      backgroundMusic.play().catch(e => console.log("Autoplay bloqueado:", e));
    }
  }, []);

  // Função auxiliar para silenciar ao sair
  const silenciarEter = () => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        if (userData.role === 'mestre') {
          ...
          if (charSnap.exists()) navigate('/jogador-vtt');
          else navigate('/create-character'); 
        }
      } else {
        // Fallback
        const charRef = doc(db, "characters", user.uid);
  ...
  const irParaAdmin = () => {
    // NÃO para a música aqui, pois o Admin Login deve ter música
    navigate('/admin-login');
  };

  return (
    <div className="login-container">
      {/* SEM TAG AUDIO AQUI - USAMOS A GLOBAL */}
